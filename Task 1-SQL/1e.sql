...
SELECT o.ownerid                                 AS "Owner Id", 
       ...
       (SELECT ( Datediff(day, TP.startdate, TP.enddate) / 
         CASE 
             WHEN (SELECT id FROM   targetrenttype t WHERE t.id = tp.paymentfrequencyid) = '1' THEN 7 
             WHEN (SELECT id FROM   targetrenttype t WHERE t.id = tp.paymentfrequencyid) = '2' THEN 14 
             ELSE 30 
         END ) * TP.paymentamount FROM   [dbo].[tenantproperty] TP WHERE  TP.[propertyid] = p.id)  AS "Total payment", 
       pf.yield                                  AS "Yield", 
       p2.firstname                              AS "Tenant Firstname", 
       p2.lastname                               AS "Tenant Lastname" 
FROM   [dbo].[property] p 
